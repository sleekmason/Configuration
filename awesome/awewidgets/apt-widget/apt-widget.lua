...
local function worker(user_args)
	...
	local icon = args.icon or ICONS_DIR .. "square-white.svg"
				...
				local refresh_button = wibox.widget({
					{
						...
						--MARGIN FOR OUTER REFRESH OF PACKAGES
						margins = 2,
						widget = wibox.container.margin,
					},
					...
					{
						{
							...
							-- SPACING BETWEEN CHECKBOX AND PACKAGE NAME
							spacing = 8,
							layout = wibox.layout.fixed.horizontal,
						},
						-- MARGINS FOR PACKAGES REFRESH
						margins = 4,
			...
			{
				{
					...
					},
					{
						{
							id = "name",
							markup = "<b>" .. #rows.children .. "</b> AVAILABLE FOR UPGRADE",
							widget = wibox.widget.textbox,
						},
						halign = "center",
						layout = wibox.container.place,
				...
				-- HEADER MARGINS FOR CHECKBOX AND REFRESH
				margins = 5,
				layout = wibox.container.margin,
			},
			bg = beautiful.bg_normal,
	...
	apt_widget:buttons(gears.table.join(awful.button({}, 1, function()
		if wibox_popup.visible then
			...
			spawn.easy_async(LIST_PACKAGES, function(stdout, stderr)
				...
				-- OFFSETs FOR WINDOW
				awful.placement.top(wibox_popup, { margins = { top = 20 }, offset = { x = -110 }, parent = mouse })
			end)
		end
	end)))
